...
from utils import construct_raw_memory_bank, construct_aggregate_memory_bank, get_new_feature
import dataset
import numpy as np
import matplotlib.pyplot as plt
import cv2

def test(num_neighbors = 3):
    raw_bank = construct_raw_memory_bank('train_val_set.txt')
    new_bank = construct_aggregate_memory_bank(raw_bank, num_neighbors)
    ...
    for test_idx, test_img in enumerate(test_loader):
        ...
        new_feature = construct_aggregate_memory_bank(raw_feature, num_neighbors)
